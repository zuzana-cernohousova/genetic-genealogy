...
def __create_settings_file(name, abs_path):
	...
	try:
		with open(os.path.join(os.path.dirname(__file__), "project_config_template.ini")) as template:
			cp.read(template)
...
def __create_global_config():
	...
	try:
		with open(os.path.join(os.path.dirname(__file__), "global_config_template.ini")) as template:
			cp.read(template)
	except IOError as err:
